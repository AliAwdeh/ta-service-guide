...
| `MAID_LOCATION` | string           | conditional | `in-country` \| `outside-country`. **Required when `NATIONALITY` is filipino, ethiopian, sri-lankan or nepali** — for those four the location selects a different guide, so it cannot be defaulted. Ethiopian and Nepali each have an in-country and an outside-country guide; Filipina and Sri Lankan are in-country-only, so `outside-country` resolves to `other`. Ignored for every other nationality. Accepts `"in_ethiopia"`, `"outside sri lanka"`, `"abroad"`, `"uae"`, `"home"`. |
